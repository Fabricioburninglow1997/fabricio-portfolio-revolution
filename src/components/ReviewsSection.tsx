...
export const ReviewsSection = ({ reviews }: ReviewsSectionProps) => {
  return (
    <section className="bg-muted py-16 dark:bg-slate-900/50 backdrop-blur-sm">
      <div className="container mx-auto px-4">
        <h2 className="text-3xl font-bold mb-12 text-center dark:text-white">Testimonios de Clientes</h2>
          ...
          <ScrollArea className="w-full rounded-lg pb-6">
            <div className="flex flex-col md:flex-row md:space-x-8 space-y-6 md:space-y-0 px-4">
              {reviews.map((review) => (
                <Card 
                  key={review.id} 
                  className="w-full md:w-[350px] md:shrink-0 group hover:shadow-xl transition-all duration-500 ease-out hover:scale-105 hover:rotate-1 dark:bg-slate-800/50 dark:border-slate-700"
                >
                  <CardContent className="p-6">
                    <div className="relative">
                      <div className="absolute -top-2 -left-2 text-4xl text-primary opacity-30">"</div>
                      <p className="text-muted-foreground mb-6 pt-4 italic dark:text-slate-300">
                        {review.comment}
                      </p>
                    </div>
                    <div className="flex items-center gap-4 mt-4 border-t dark:border-slate-700 pt-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground dark:text-slate-400">{review.role}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
